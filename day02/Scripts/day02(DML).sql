...
--VALUES (2, '이유리', 89, 91, 77);
--VALUES (3, '김웅이', 48, 100, 100);
--VALUES (4, '홍길동', 70, 25, 55);
--VALUES (5, '신짱구', 50, 10);

SELECT * FROM TBL_STUDENT;


SELECT STUDENT_NAME 이름, (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3 
FROM TBL_STUDENT;
...
-- ROUND(값, 자리수) : ROUND 함수는 소수점을 반올림하여 특정 수소점 까지만 표현할 수 있다
-- ROUND(평균 구하는 공식 >= 90)
UPDATE TBL_STUDENT 
SET STUDENT_GRADE  = 'A'
...
SELECT STUDENT_NAME 이름, (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3 평균, STUDENT_GRADE 학점
FROM TBL_STUDENT ts ;
...
FROM TBL_STUDENT;
WHERE  (STUDENT_MATH >= 50, AND STUDENT_KOR >= AND, STUDENT_ENG >=50) AND (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3 >=80
