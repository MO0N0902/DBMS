...
SELECT *FROM TBL_BOOKPRICE tb ;
SELECT *FROM TBL_BOOKS tb2;

--1) 왼쪽 외부조인
-- BOOKS 테이블 기준 왼쪽 외부조인을 수행하고 BOOKSPRICE 테이블의 데이터를 포함(왼쪽 테이블의 데이터가 없어도 결과에 포함됨)
SELECT TB.BOOK_ID , TB.BOOK_TITLE ,TB.BOOK_AUTHOR ,TB2.BP_ID ,TB2.BP_PRICE 
FROM TBL_BOOKS tb LEFT OUTER JOIN TBL_BOOKPRICE tb2
ON TB.BOOK_ID = TB2.BP_ID ;

--2) 오른쪽 외부조인(RIGHT OUTER JOIN)
--BOOKPRICE 기준으로 오른쪽 외부조인을 수행하고 
SELECT TB.BOOK_ID , TB.BOOK_TITLE ,TB.BOOK_AUTHOR ,TB2.BP_ID ,TB2.BP_PRICE 
FROM TBL_BOOKS tb RIGHT OUTER JOIN TBL_BOOKPRICE tb2
ON TB.BOOK_ID = TB2.BP_ID ;
...
SELECT TB.BOOK_ID , TB.BOOK_TITLE , TB.BOOK_AUTHOR, TB2.BP_ID ,TB2.BP_PRICE 
FROM TBL_BOOKS tb FULL OUTER JOIN TBL_BOOKPRICE tb2
ON TB.BOOK_ID = TB2.BP_ID ;
